    ...
    $.gw2tooltip = function(selector, settings){
        ...
        var injectStyle = function(data) {
                ...
                switch (data.type.toLowerCase()) {
                    case('weapon'):
                        return $(document.createElement('div')).addClass('weapon').append([
                            $(document.createElement('div')).addClass('weaponStrength').text(translate('weaponstrength')+' '+data.weapon.min_power+translate('demageseparator')+data.weapon.max_power),
                            attributesList('weapon'),
                            $(document.createElement('div')).addClass('weaponType').text(data.weapon.type)
                        ]);
                    case('armor'):
                        return $(document.createElement('div')).addClass('armor').append([
                            $(document.createElement('div')).addClass('defense').text(translate('defense')+' '+data.armor.defense),
                            attributesList('armor'),
                            $(document.createElement('div')).addClass('weightClass').text(translate(data.armor.weight_class.toLowerCase())),
                            $(document.createElement('div')).addClass('armorType').text(translate(data.armor.type.toLowerCase()))
                        ]);
            ...
            $gw2tooltip.addClass(config.mode).addClass(data.rarity.toLowerCase()+'Item').html($bodyWrapper.html([
                /* $(document.createElement('img')).attr({height:32,width:32,alt:data.name,src:data.}) no image url :( */
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if(config.localStorage && isStorageActiv) {
                        ...
                        activeStorage[data.item_id] = data;
